...
from mayhem.proc.windows import WindowsProcess, get_pid_from_name
...
PIPE_NAME = 'mayhem-{}'.format(RAND_ID)
...
def main():
        ...
        parser.add_argument('sncript_path', action='store', help='python script to inject into the process')
        parser.add_argument('ident', action='store', help='process name or pid')
        parser.epilog = 'The __name__ variable will be set to "__mayhem__".'
        arguments = parser.parse_args()

        if not sys.platform.startswith('win'):
                print('[-] This tool is oly available on Windows')
                return
        ...
        try:
                ...
                if type(ident) == int:
                        ...
                        pid = get_pid_from_name(ident)
                process_h = WindowsProcess(pid=pid)
        except ProcessError as error:
                print("[-] {0}".format(error.msg))
                return
        print("[+] Opened a handle to pid: {0}".format(pid))
